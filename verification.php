...
<html>
	<head>
		<title><?php print T_("Vérification");?></title>
	...
	if( isset($_GET['p']) && isset($_GET['c']) ){
		$pseudo=htmlspecialchars($_GET['p']);
		...
		if($count == 1){
			$reponse = $bdd->prepare("SELECT id FROM `votresecretgard`.`table_utilisateur` WHERE `table_utilisateur`.`cle` = :cle ");
			$reponse->execute(array(':cle' => $cle));
			$count = $reponse->rowCount();
			if($count == 1){
				$verou = 1;
				$request=$bdd->prepare('UPDATE `votresecretgard`.`table_utilisateur` SET `verif` = 1 WHERE `table_utilisateur`.`cle` = :cle ');
				$request->execute(array(':cle' => $cle));
				$reponse = $bdd->prepare("CREATE TABLE ".$pseudo." LIKE `votresecretgard`.`baton`");
				$reponse->execute();
			}
			else{
											...
											<div class="8u skel-cell-important">
												...
												<!-- Content -->
													<article class="box is-post">
														...
														<header>
															<h2><?php print T_("Bienvenue sur Secret Garden et félicitations !");?></h2>
															<span class="byline"><?php print T_("validation de votre compte.");?></span>
														</header>
														<p><?php print T_("E-mail validé, vous allez être redirigé vers la page de connexion.");?></p>
														<p><?php print T_("Ps : nous sommes ravis que vous soyez un humain ;-)");?> </p>
														<p><?php print T_("Vous allez être redirigé vers la page de connexion.");?> </p>
														<a href="login.php"><?php print T_("Si ce n'est pas le cas au bout de 30s cliquez ici.");?> </a>
														<?php header ("Refresh: 30 ;URL=login.php"); ?>
													</article>
											</div>
										</div>
									</div>
